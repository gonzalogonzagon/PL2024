...
defines [bool first]returns [Constantes cons]: '#define' {Elemento elem = new Elemento();} IDENT {elem.setId($IDENT.text)} ctes {elem.setTipo($ctes.s); cons.anadirElemento(elem)} defines[false]
    {$cons = }
    | {$s = "";}
    ;
